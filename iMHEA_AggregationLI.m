function [NewDate,NewP,CumP,Single] = iMHEA_AggregationLI(Event_Date,Event_mm,scale,bucket,mintip,halves,varargin)
%iMHEA Linear interpolation rainfall aggregation.
% [NewDate,NewP,CumP,Single] =
% iMHEA_AggregationLI(Event_Date,Event_mm,scale,Vb,mintip,halves,flag)
% aggregates precipitation data using linear interpolation.
%
% Input:
...
fprintf('\n')
fprintf('RAINFALL AGGREGATION USING LINEAR INTERPOLATION.\n')
...
% Restore initial extreme to avoid crashing the code later.
NewEvent_Date = cat(1,Event_Date(1),NewEvent_Date);
...
fprintf('Average duration of the events between registered tips: %8.2f min.\n',mean(D(D>0)))
...
for i = 1:length(n)
    % Events with more than or equal to 2 points (fit a line) [Ciach et al, 2003].
    ...
    if n(i) >= 1
        ...
        y1m = interp1(x,y,x1m,'linear',0); % Cumulative rainfall at each x1m
        if halves ~= false || halves ~= 0
        ...
        [r2m,y2m,biased(i),bEvent(i)] = intCorrection(r1m,y(end),Lowint,halves);
        ...
        if nargin > 6 || any(isnan(r2m)) %|| bEvent(i)>0
            ...
            figure(197)
            subplot(2,1,1)
            plot(x,y,'o',x1m,y5m,':',x1m,y1m,'-.',x1m,y2m,'-.')
            ...
            legend('Events','1-min tip counting',...
                '1-min linear interpolated','1-min linear corrected',...
                'location','NorthWest')
            legend('boxoff')
            subplot(2,1,2)
            plot(x,60*[y(1);diff(y)],'o',x1m,60*r5m,':',x1m,60*r1m,'-.',x1m,60*r2m,'-.')
            set(gca,'Xlim',[-60 x(end)+60])
            xlabel('Time in seconds from the beggining of the event [s]')
            ylabel('Rainfall rate [mm h^{-1}]')
            legend('Events','1-min tip counting','1-min linear interpolated','1-min linear corrected',...
                'location','NorthWest')
            legend('boxoff')
        ...
        x = (xf-x0*nd/1440:xf)'; % Equally spaced divided tip time [min]
...
DI = floor(min(Event_Date)*nd)*scale; % Initial date in [min]
...
% Add additional tips for long periods.
% Calculate the time between tips.
Diff_Event_Date = diff(Event_Date);
...
function [r2m,y2m,biased,bEvent] = intCorrection(r1m,Total,Lowint,halves)
% Only keep rain rates above the threshold.
r2m = r1m;
bEvent = 0;
biased = abs(Total-sum(r1m(r1m>0)))/Total;
if biased > 0.25
    bEvent = 1;
end
iter = 0;
while iter<= 10 && (abs(Total-sum(r2m))>Lowint || any(round(r2m(r2m~=0),8) < Lowint))
    iter = iter + 1;
