...
%iMHEA entire workflow for a catchment.
...
DataHRes = nan(length(DateAux),3);
DataHRes(:,1) = DateAux;
DataHRes(ismember(DataHRes,round(DateP_HRes*nd)),2) = P_HRes;
DataHRes(ismember(DataHRes,round(DateQ_HRes*nd)),3) = Q_HRes;
% Rescale the date
